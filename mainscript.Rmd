...
#################
# 11.11.2021
# Separate Bullock's mod/hist and Baltimore mode/hist
update <- alldat.orn[which(alldat.orn$cat == "Bull_mod" | alldat.orn$cat == "Bull_hist" | alldat.orn$cat == "Balt_mod" | alldat.orn$cat == "Balt_hist"),]

# List for comparisons
updatelist <- list(c("Bull_hist", "Bull_mod"), c("Balt_hist", "Balt_mod"), c("Bull_hist", "Balt_hist"), c("Bull_mod", "Balt_mod"))


############
# do black patches
update1 <- alldat.blk[which(alldat.blk$cat == "Bull_mod" | alldat.blk$cat == "Bull_hist" | alldat.blk$cat == "Balt_mod" | alldat.blk$cat == "Balt_hist"),]

# List for comparisons
updatelist1 <- list(c("Bull_hist", "Bull_mod"), c("Balt_hist", "Balt_mod"), c("Bull_hist", "Balt_hist"), c("Bull_mod", "Balt_mod"))
```
...
#####
h2a1 <- ggplot(update, aes(x = cat, y = B1, fill = sp)) +
  stat_boxplot(aes(x = cat, y = B1), geom = "errorbar", position = position_dodge(width = .75), width = .5) +
  geom_boxplot(outlier.size = 1.5, position = position_dodge(width = .75), col = "black") +
  theme_classic()+ 
  stat_compare_means(comparisons = updatelist, method = "t.test", label = "p.signif", size = 6) + 
  mytheme +
  labs(y = "Total brightness (B1)") +
  scale_fill_manual(values = pal1, name = "Species", limits = c("Bull", "Balt"), labels = c(expression(italic("Icterus bullockii")), expression(italic("Icterus galbula")))) +
  scale_x_discrete(name = "Specimen type", limits = c("Bull_hist", "Bull_mod", "Balt_hist", "Balt_mod"), labels = c("Historic", "Modern","Historic", "Modern")) +
  ylim(7500,12600) +
...
### black
h2blk <- ggplot(update1, aes(x = cat, y = B1, fill = sp)) +
  stat_boxplot(aes(x = cat, y = B1), geom = "errorbar", position = position_dodge(width = .75), width = .5) +
  geom_boxplot(outlier.size = 1.5, position = position_dodge(width = .75), col = "black") +
  theme_classic()+ 
  stat_compare_means(comparisons = updatelist1, method = "t.test", label = "p.signif", size = 6) + 
  mytheme +
  labs(y = "Total brightness (B1)") +
  scale_fill_manual(values = pal1, name = "Species", limits = c("Bull", "Balt"), labels = c(expression(italic("Icterus bullockii")), expression(italic("Icterus galbula")))) +
  scale_x_discrete(name = "Specimen type", limits = c("Bull_hist", "Bull_mod", "Balt_hist", "Balt_mod"), labels = c("Historic", "Modern","Historic", "Modern")) +
  theme(legend.text.align = 0)
h2blk
ggsave("Figure_h2_blk.png", plot=h2blk, width=200, height=125, units="mm", dpi=600, scale=T)
```
...
#############
h2b1 <- ggplot(update, aes(x = cat, y = S9, fill = sp)) +
  stat_boxplot(aes(x = cat, y = S9), geom = "errorbar", position = position_dodge(width = .75), width = .5) +
  geom_boxplot(outlier.size = 1.5, position = position_dodge(width = .75), col = "black") +
  theme_classic()+ 
  stat_compare_means(comparisons = updatelist, method = "t.test", label = "p.signif", size = 6) + 
...
# remove ref
h2c1 <- ggplot(update, aes(x = cat, y = H3, fill = sp)) +
  stat_boxplot(aes(x = cat, y = H3), geom = "errorbar", position = position_dodge(width = .75), width = .5) +
  geom_boxplot(outlier.size = 1.5, position = position_dodge(width = .75), col = "black") +
  theme_classic()+ 
  stat_compare_means(comparisons = updatelist, method = "t.test", label = "p.signif", size = 6) + 
  ...
  scale_fill_manual(values = pal1, name = "Species", limits = c("Bull", "Balt"), labels = c(expression(italic("Icterus bullockii")), expression(italic("Icterus galbula")))) +
  scale_x_discrete(name = "Specimen type", limits = c("Bull_hist", "Bull_mod", "Balt_hist", "Balt_mod"), labels = c("Historic", "Modern","Historic", "Modern")) +
  theme(legend.text.align = 0, legend.position = c(0.7, 0.15), legend.background = element_rect(linetype="solid", 
                                  colour ="lightgray")) 
...
#########
pg2 <- plot_grid(
  h2a1 + theme(legend.position="none"),
  h2b1 + theme(legend.position="none"),
  h2c1,
  align = 'vh',
  labels = c("A", "B", "C"),
  hjust = -1,
...
ggsave("Figure_h2_v2.png", plot=last_plot(), width=400, height=125, units="mm", dpi=600, scale=T)
```
...
ggplot(bullblk, aes(x = date, y = B1)) +
  ...
  scale_color_manual(values = pal1) +
  scale_fill_manual(values = pal1)
ggsave("black_B1_time.png", plot=last_plot(), width=200, height=125, units="mm", dpi=600, scale=T)
```
...
ggplot(vod2, aes(x=Var1, y=dS.mean)) +
  geom_errorbar(aes(ymin = dS.lwr, ymax = dS.upr), width=.4, position = position_dodge(width=1), size=1) +
...
# select comparisons
vod3 <- vod2 %>% 
  filter(Var1 == "Bull_hist-Bull_mod" | Var1 == "Balt_hist-Balt_mod" | Var1 == "Balt_hist-Bull_hist" | Var1 == "Balt_mod-Bull_mod")

ggplot(vod3, aes(x=Var1, y=dS.mean)) +
  geom_errorbar(aes(ymin = dS.lwr, ymax = dS.upr), width=.3, position = position_dodge(width=1), size=1) +
  geom_point(position = position_dodge(width=1), size=5) +
  theme_classic()+ 
  mytheme + 
  theme(text=element_text(family="sans")) +
  ...
  theme(panel.background = element_rect(fill = "transparent"), # bg of the panel
    ...
    legend.background = element_rect(fill = "transparent"), # get rid of legend bg
    legend.box.background = element_rect(fill = "transparent"))+
  scale_x_discrete(limits = c("Balt_mod-Bull_mod", "Balt_hist-Bull_hist", "Balt_hist-Balt_mod", "Bull_hist-Bull_mod"), 
                   labels = c("Interspecific modern", "Interspecific historic", expression(paste("Intraspecific ", italic("Icterus galbula"))), expression(paste("Intraspecific ", italic("Icterus bullockii")))))
ggsave("chromatic_contrast1.png", plot = last_plot(), width = 200, height = 80, units = "mm", dpi = 300,  bg = "transparent")
...
ggplot(vod3, aes(x=Var1, y=dS.mean)) +
  ...
  theme(panel.background = element_rect(fill = "transparent"), axis.title.y = element_text(angle = 0, hjust = 0), # bg of the panel
    plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
    panel.grid.major = element_blank(), # get rid of major grid
    panel.grid.minor = element_blank(), # get rid of minor grid
    legend.background = element_rect(fill = "transparent"), # get rid of legend bg
  ...
  scale_x_discrete(limits = c("Balt_mod-Bull_mod", "Balt_hist-Bull_hist", "Balt_hist-Balt_mod", "Bull_hist-Bull_mod"), 
                   labels = c("Interspecific modern", "Interspecific historic", expression(paste("Intraspecific ", italic("Icterus galbula"))), expression(paste("Intraspecific ", italic("Icterus bullockii"))))) +
  geom_text(x=4.5, y=0.2, inherit.aes = F, label = "Centroid comparison", check_overlap = T, hjust =1, fontface="bold", size=7)
ggsave("chromatic_contrast2.png", plot = last_plot(), width = 200, height = 80, units = "mm", dpi = 300,  bg = "transparent")
...
##############
voda <- as.data.frame(as.table(vm.orn.dist.achro))
voda1 <- pivot_wider(voda, names_from = Var2, values_from = Freq)

voda2 <- voda1 %>% 
  filter(Var1 == "Bull_hist-Bull_mod" | Var1 == "Balt_hist-Balt_mod" | Var1 == "Balt_hist-Bull_hist" | Var1 == "Balt_mod-Bull_mod")


ggplot(voda2, aes(x=Var1, y=dL.mean)) +
  geom_errorbar(aes(ymin = dL.lwr, ymax = dL.upr), width=.3, position = position_dodge(width=1), size=1) +
  geom_point(position = position_dodge(width=1), size=5) +
  theme_classic()+ 
  mytheme + 
  theme(text=element_text(family="sans")) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  coord_flip(clip="off") +
  labs(x=NULL, y = "Achromatic contrast (\u0394L)") +
  theme(panel.background = element_rect(fill = "transparent"), axis.title.y = element_text(angle = 0, hjust = 0), # bg of the panel
    plot.background = element_rect(fill = "transparent", color = NA), # bg of the plot
    panel.grid.major = element_blank(), # get rid of major grid
    panel.grid.minor = element_blank(), # get rid of minor grid
  ...
  scale_x_discrete(limits = c("Balt_mod-Bull_mod", "Balt_hist-Bull_hist", "Balt_hist-Balt_mod", "Bull_hist-Bull_mod"), 
                   labels = c("Interspecific modern", "Interspecific historic", expression(paste("Intraspecific ", italic("Icterus galbula"))), expression(paste("Intraspecific ", italic("Icterus bullockii"))))) +
  geom_text(x=4.55, y=-0.1, inherit.aes = F, label = "Centroid comparison", check_overlap = T, hjust =1, fontface="bold", size=7)
ggsave("achromatic_contrast2.png", plot = last_plot(), width = 200, height = 80, units = "mm", dpi = 300,  bg = "transparent")
...
Plot brightness overtime
```{r}
orn1 <- ggplot(bullocks, aes(x = date, y = B1)) +
  geom_point(size = 2) +
  stat_smooth(method = lm, se = T, color="#ffa600", fill="#ffa600") +
  theme_classic() +
  mytheme +
  stat_fit_glance(method = "lm", 
                        method.args = list(formula = y ~ x), size = 5, 
                        aes(label = sprintf('R^2~"="~%.3f~~italic(p)~"="~%.3g',
                                            stat(..r.squared..), stat(..p.value..))), parse = TRUE) + 
  labs(y = "Orange patches - total brightness (B1)", x= "Collection date") +
  scale_color_manual(values = pal1) +
  scale_fill_manual(values = pal1)
...
blk1 <- ggplot(bullblk, aes(x = date, y = B1)) +
  geom_point(size = 2) +
  stat_smooth(method = lm, se = T, color="darkgray", fill="darkgray") +
  theme_classic() +
  mytheme +
  stat_fit_glance(method = "lm", 
                        method.args = list(formula = y ~ x), size = 5, 
                        aes(label = sprintf('R^2~"="~%.3f~~italic(p)~"="~%.3g',
                                            stat(..r.squared..), stat(..p.value..))), parse = TRUE) + 
  labs(y = "Black patches - total brightness (B1)", x= "Collection date") +
  scale_color_manual(values = pal1) +
  scale_fill_manual(values = pal1)

pg_time <- plot_grid(
  orn1 + theme(legend.position="none"),
  blk1 + theme(legend.position="none"),
  align = 'vh',
  labels = c("A", "B"),
  hjust = -1,
  nrow = 1, label_size = 16
...
ggsave("Figure_time_B1.png", plot=last_plot(), width=300, height=125, units="mm", dpi=600, scale=T)
```
